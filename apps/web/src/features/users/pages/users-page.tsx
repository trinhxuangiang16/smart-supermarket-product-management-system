  ...
  if (!canManageUsers) {
    return (
      <div className="space-y-4">
        <Card>
          <p className="text-sm text-[#6d5935]">
    ...
    <div className="space-y-4">
      ...
      <Card>
        <div className="grid gap-2 md:grid-cols-4">
          ...
          <select className="field-warm h-11 rounded border px-3 text-sm outline-none focus:ring-2 focus:ring-[#bb9645]/25" value={roleFilter} onChange={(e) => { setRoleFilter(e.target.value); setPage(1); }}>
            <option value="">All roles</option>
            {roleOptions.map((role) => <option key={role} value={role}>{role}</option>)}
          </select>
          <select className="field-warm h-11 rounded border px-3 text-sm outline-none focus:ring-2 focus:ring-[#bb9645]/25" value={activeFilter} onChange={(e) => { setActiveFilter(e.target.value); setPage(1); }}>
            <option value="">All statuses</option>
            <option value="true">Active</option>
            <option value="false">Inactive</option>
          </select>
          <select className="field-warm h-11 rounded border px-3 text-sm outline-none focus:ring-2 focus:ring-[#bb9645]/25" value={String(pageSize)} onChange={(e) => { setPageSize(Number(e.target.value)); setPage(1); }}>
      ...
      <Card>
        ...
        <div className="grid gap-2 md:grid-cols-4">
          ...
          <select className="field-warm h-11 rounded border px-3 text-sm outline-none focus:ring-2 focus:ring-[#bb9645]/25" value={createForm.role} onChange={(e) => setCreateForm({ ...createForm, role: e.target.value })}>
      ...
      <Card>
        <div className="overflow-auto">
          <table className="table-warm w-full min-w-[980px] table-fixed text-sm">
            <colgroup>
              <col className="w-[20%]" />
              <col className="w-[22%]" />
              <col className="w-[18%]" />
            ...
            <tbody>
              {usersQuery.isLoading ? (
                <tr><td colSpan={6} className="px-3 py-8 text-center text-muted-warm">Loading users...</td></tr>
              ) : null}
              {!usersQuery.isLoading && !rows.length ? (
                <tr><td colSpan={6} className="px-3 py-8 text-center text-muted-warm">No users found.</td></tr>
              ) : null}
              {rows.map((u: any) => (
                <tr key={u.id}>
        ...
        <div className="mt-3 flex items-center justify-between text-sm">
          <span className="text-muted-warm">Total users: {total}</span>
      ...
      {editingUser ? (
        <div className="fixed inset-0 z-40 grid place-items-center bg-[#15371f]/45 p-4">
          <div className="surface-card w-full max-w-xl rounded-md border shadow-2xl">
            <div className="flex items-start justify-between border-b border-[var(--color-border)] px-5 py-4">
              <div>
                <h2 className="text-lg font-semibold">Edit User</h2>
                <p className="text-sm text-muted-warm">{editingUser.email}</p>
              </div>
              <button className="btn-muted-warm grid h-9 w-9 place-items-center rounded" onClick={() => setEditingUser(null)}>
                <X size={16} />
              </button>
            </div>
            <div className="space-y-3 p-5">
              <Input value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} />
              <select className="field-warm h-11 w-full rounded border px-3 text-sm outline-none focus:ring-2 focus:ring-[#bb9645]/25" value={editForm.role} onChange={(e) => setEditForm({ ...editForm, role: e.target.value })}>
            ...
            <div className="flex justify-end gap-2 border-t border-[var(--color-border)] bg-[#f7ebd5] px-5 py-4">
              <button className="btn-muted-warm h-11 rounded px-4 text-sm" onClick={() => setEditingUser(null)}>Cancel</button>
          ...
          <div className="surface-card w-full max-w-xl rounded-md border shadow-2xl">
            <div className="flex items-start justify-between border-b border-[var(--color-border)] px-5 py-4">
              <div>
                ...
                <p className="text-sm text-muted-warm">{passwordModal.email}</p>
              </div>
              <button className="btn-muted-warm grid h-9 w-9 place-items-center rounded" onClick={() => setPasswordModal(null)}>
                <X size={16} />
              </button>
            </div>
            <div className="space-y-3 p-5">
              <Input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} />
              <p className="text-xs text-muted-warm">Minimum 8 characters.</p>
            </div>
            <div className="flex justify-end gap-2 border-t border-[var(--color-border)] bg-[#f7ebd5] px-5 py-4">
              <button className="btn-muted-warm h-11 rounded px-4 text-sm" onClick={() => setPasswordModal(null)}>Cancel</button>
              <Button
      ...
      {confirmModal ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-[#15371f]/45 p-4">
          <div className="surface-card w-full max-w-lg rounded-md border shadow-2xl">
            <div className="border-b border-[var(--color-border)] px-5 py-4">
              <h2 className="text-lg font-semibold">Confirm Action</h2>
              <p className="mt-1 text-sm text-[#6d5935]">
            ...
            <div className="flex justify-end gap-2 border-t border-[var(--color-border)] bg-[#f7ebd5] px-5 py-4">
              <button className="btn-muted-warm h-11 rounded px-4 text-sm" onClick={() => setConfirmModal(null)}>Cancel</button>
              <Button
                className={confirmModal.action === "delete" ? "btn-danger-warm" : ""}
